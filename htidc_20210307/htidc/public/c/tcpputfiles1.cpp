...
// 客户端登录服务器
bool ClientLogin(const char *argv,char *username,char *password);
...
int main(int argc,char *argv[])
{
 ...
 // CloseIOAndSignal();
  ...
  {             
    ...
    if(ClientLogin(argv[2],username,password)==false) return -1;
  // if(ClientLogin(argv[2])==false) return -1;
    // 实现文件发送的功能
    _tcpputfiles();
...
void _help()
{
  ...
  printf("Sample:/home/li49/htidc/public/c/tcpputfiles1 /home/li49/htidc/tcpputfiles1_surfdata1.log \"<ip>49.232.185.49</ip><port>5006</port><ptype>1</ptype><clientpath>/home/li49/data</clientpath><clientpathbak>/data/shqx/sdata/surfdatabak</clientpathbak><srvpath>/home/li49/htidc/datasrv</srvpath><andchild>true</andchild><matchname>SURF_*.TXT,*.DAT</matchname><okfilename>/home/li49/htidc/public/tcpputfilessurfdata1.xml</okfilename><timetvl>10</timetvl>\"\n\n\n");
  ...
  while(true)
   {
    ...
    sprintf(userinfo,"<username>%s</username><password>%s</password><clienttype>1</clienttype>",username,password);
     strcat(strSendBuffer,userinfo);
   //  strcat(strSendBuffer,"<clienttype>1</clienttype>");
     ...
     //if(strcmp(strRecvBuffer,"failed")==0) printf("用户名或密码错误，请重新登录!\n");return -1;
     break;
   }
   if(strcmp(strRecvBuffer,"failed")==0) {printf("用户名或密码错误，请重新登录!\n");return false;}
