...
int main(int argc,char *argv[])
{
  if (argc != 4)
  {
    ...
    printf("此程序调用df命令，把本服务器的磁盘使用率信息写入xml文件。\n");
  ...
  while (true)
  {
    ...
    if (strncmp(strBuffer,"/dev",4) != 0) continue;//比较str1和str2前n个字符的大小。

    CmdStr.SplitToCmd(strBuffer," ");//将一行内容按空格进行拆分

    if (CmdStr.CmdCount() != 6) continue;//拆分块不等于6

    char strusep[21];
    memset(strusep,0,sizeof(strusep));//     /dev/sda  1014   136   879   14%  /boot
