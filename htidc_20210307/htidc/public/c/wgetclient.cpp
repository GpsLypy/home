...
int main(int argc, char *argv[])
{
  ...
  snprintf(strfilenametmp,300,"%s.tmp",argv[3]);//把正式文件名修改成一个临时文件名，避免输出的中间过程被读取

   // 把获取到的网页转换为中文（把临时文件里的内容转为中文，然后输出到正式的临时文件中去）
  memset(strcmd,0,sizeof(strcmd));
  ...
  RENAME(strfilenametmp,argv[3]);//将临时文件名修改为正式文件
