...
struct st_arg
{
  ...
  int  mode;             //传输模式，1-被动模式，2-主动模式，缺省采用被模式。
  char username[31];     //远程服务器FTP的用户名
  char password[31];     //远程服务器FTP的密码
  char localpath[301];   //本地文件存放的目录
  char remotepath[301];  //远程服务器存放文件的目录
  char matchname[301];   //待发送文件匹配的文件名，采用大写匹配
  int  ptype;            //不匹配的文件不会被发送，本字段尽可能设置精确，不允许用*匹配全部的文件
  char localpathbak[301];//文件发送成功后，本地文件的处理方式：1-什么也不做；2-删除；3-备份，如果为3,还要指定备份的目录
  char okfilename[301];  //已发送成功文件名清单，此参数只有当ptype=1时有效
  int  timetvl;          //发送时间间隔，单位：秒，建议大于10
} starg;

void EXIT(int sig);

Cftp ftp;//ftp操作的类
CLogFile logfile;//日志文件操作的类
...
bool _ftpputfiles()
{
  // 创建远程服务器上的目录，不关心返回值,如果对方给了权限，可以创建成功，没给权限，对方应该是建立好了的
  ...
  if (starg.ptype==1)//如果要求本地文件的处理方式为什么也不做，也就是”减量“”传输，不发送已经发送过的文件
  {
  ...
  for (int ii=0;ii<vlistfile.size();ii++)
  {
    ...
    if (starg.ptype==3)
     {
       char strfilenamebak[301];//用来存放备份目录名和文件
...
void _help(char *argv[])
{
  ...
  printf("<okfilename>/data/shqx/ftplist/ftpputfiles_surfdata.xml</okfilename> 已发送成功文件名清单，此参数只有当ptype=1时有效。\n");
