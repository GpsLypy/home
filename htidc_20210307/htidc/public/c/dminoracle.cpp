...
bool _dmintables()
{
  ...
  stmt.prepare(strsql);//fieldname 挖掘字段的别名列表
  ...
  int  iFileSeq=1;   // 待生成文件的序号,用来标记文件
  ...
  while (true)
  {
    ...
    for (int ii=0;ii<vfieldname.size();ii++)
    {
      File.Fprintf("<%s>%s</%s>",vfieldname[ii].c_str(),fieldvalue[ii],vfieldname[ii].c_str());//输出变量定义为一个二维数组
    }
    File.Fprintf("<endl/>\n");
...
bool LoadMaxKeyid()
{
  ...
  {
    logfile.Write("注意，%s文件不存在，程序将重新开始挖掘数据。\n",starg.idfilename); return true;
  }
