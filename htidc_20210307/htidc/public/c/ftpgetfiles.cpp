...
vector<struct st_fileinfo> vlistfile,new_vlistfile;
...
// 二、在vlistfile中存在，新文件或需要重新采集的文件new_vlistfile
bool CompVector();
...
bool _ftpgetfiles()
{
  ...
  // 发送nlist命令列出ftp服务器目录中的文件，结果保存到本地文件listfilename中
  ...
  if (starg.ptype==1)
  {
    ...
    // 把new_vlistfile的内容复制到vlistfile容器中
    vlistfile.clear(); vlistfile.swap(new_vlistfile);
  }
  ...
  for (int ii=0;ii<vlistfile.size();ii++)
  {
    ...
    if (starg.ptype==3)
    {
      ...
      SNPRINTF(strremotefilenamebak,301,300,"%s/%s",starg.remotepathbak,vlistfile[ii].filename);
...
// 把用nlist方法获取到的list文件中的符合要求的文件加载到vlistfile容器中
bool LoadListFile()
{
  vlistfile.clear();
  ...
  if (File.Open(starg.listfilename,"r") == false)//打开本地用来存放服务器目录下的文件清单的文件
  {
    logfile.Write("File.Open(%s) 失败。\n",starg.listfilename); return false;
  }

  struct st_fileinfo stfileinfo;//文件信息的数据结构,用来存放文件名和文件时间
  ...
  while (true)
  {
    ...
    if (starg.ptype==1)//匹配一个文件成功后，相当于成功获取服务器目录下的一个文件，接着判断对服务器上的文件采取的处理方式
    {
      // 获取对方服务器上这个正确文件的时间，匹配到的这个文件和服务器上的文件名相同
...
bool CompVector()
{
  vokfilename1.clear();  new_vlistfile.clear();
  ...
  for (int ii=0;ii<vlistfile.size();ii++)
  {
    ...
    {
      new_vlistfile.push_back(vlistfile[ii]);//当程序第一次运行，到这里时，意思就是说vlistfile容器里面的文件全部需要采集
    }
  }
  ...
  {
    logfile.Write("new_vlistfile filename=%s,mtime=%s\n",vlistfile1[ii].filename,vlistfile1[ii].mtime);
  }
  */
...
void _help(char *argv[])
{
    ...
    printf("Sample:/htidc/public/bin/ftpgetfiles /log/shqx/ftpgetfiles_surfdata.log \"<host>192.168.65.130:21</host><port>21</port><mode>1</mode><username>oracle</username><password>liyupeng66</password><localpath>/data/shqx/sdata/surfdata</localpath><remotepath>/data/shqx/ftp/surfdata</remotepath><matchname>SURF_*.TXT,*.DAT</matchname><ptype>1</ptype><remotepathbak>/home/li49/qxidc/data/ftp/surfdatabak</remotepathbak><listfilename>/data/shqx/ftplist/ftpgetfiles_surfdata.list</listfilename><okfilename>/data/shqx/ftplist/ftpgetfiles_surfdata.xml</okfilename><timetvl>30</timetvl>\"\n\n\n");

    printf("本程序是数据中心的公共功能模块，用于把远程FTP服务器的文件采集到本地目录。\n");
    printf("logfilename是本程序运行的日志文件。\n");
    printf("xmlbuffer为文件传输的参数，如下：\n");
    printf("<host>49.232.185.49:21</host> 远程服务器的IP和端口。\n");
    printf("<mode>1</mode> 传输模式，1-被动模式，2-主动模式，缺省采用被模式。\n");
    printf("<username>li49</username> 远程服务器FTP的用户名。\n");
    printf("<password>li498988598</password> 远程服务器FTP的密码。\n");
    printf("<localpath>/oracle/qxidc/data/ftp/surfdata</localpath> 本地文件存放的目录。\n");
    printf("<remotepath>/home/li49/qxidc/data/ftp/surfdata</remotepath> 远程服务器存放文件的目录。\n");
    ...
    printf("<listfilename>/oracle/qxidc/list/ftpgetfiles_surfdata.list</listfilename>采集前把列出的服务器里的文件保存到这个文件。\n");
    ...
    //实现文件的增量采集，把已经采集过的文件用一个xml文件保存起来，每次拿到服务器上的文件清单，就和list文件对比
