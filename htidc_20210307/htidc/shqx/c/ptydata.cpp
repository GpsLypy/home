/*
 *  本程序用于处理台风数据，并保存到数据库的T_tycode t_typath表中。
*/
#include "_public.h"
#include "_ooci.h"
#include "_shqx.h"
...
int main(int argc,char *argv[])
{
  if (argc!=5)
  {
    ...
    printf("例如：/htidc/shqx/bin/ptydata /data/shqx/sdata/TYDATA /log/shqx/ptydata.log shtqapp/pwdidc 10\n");
    return -1;
  }
...
bool _ptydata()
{
  ...
  logfile.WriteEx("此文件中记录总数为sum=%d\n",TYDATA.sum);

  logfile.WriteEx("此文件中头记录表处理成功(total1=%d,insert1=%d,update1=%d,invalid1=%d)。\n",TYDATA.totalcount1,TYDATA.insertcount1,TYDATA.updatecount1,TYDATA.invalidcount1);

  logfile.WriteEx("此文件中实况路径表处理成功(total2=%d,insert2=%d,update2=%d,invalid2=%d)。\n",TYDATA.totalcount2,TYDATA.insertcount2,TYDATA.updatecount2,TYDATA.invalidcount2);
