...
int main(int argc,char *argv[])
{
  if (argc!=7)
  {
    ...
    printf("例如：/htidc/shqx/bin/pzhrain24file /log/shqx/pzhrain24file.log  girla/girlpwdaa@Tencent11g_49.232.185.49 /data/wfile/zhrain24 data/qxfile/zhrain24 T_ZHRAIN24 30\n");
...
bool _pzhrain24file(char *strargv2,char *strargv4,char *strargv5)
{
  //对于一个(逐小时降雨量)文件，我们区分它的一个显著标志就是数据时间
  char strddatetime[21];//用来存放文件的数据时间，格式yyyymmddhh24miss
  memset(strddatetime,0,sizeof(strddatetime));
  strncpy(strddatetime,strstr(Dir.m_FileName,"20"),14);//PWCP_TWC_WEAP_SFER_ER1_TWC_L88_P9_20210106180000000.JPG
  
  //要把处理完的文件放到一个目标目录下，所以先对文件重命名以方便区分
  char strdstfilename[301];//目标文件名，不带路径
  memset(strdstfilename,0,sizeof(strdstfilename));
  snprintf(strdstfilename,300,"zhrain24_%s.jpg",strddatetime);//"格式化"输出到字符串
  
  //对文件的目标目录结构进行合理组织
  char strdstfilepath[301];//目标文件存放的目录
  memset(strdstfilepath,0,sizeof(strdstfilepath));
  snprintf(strdstfilepath,300,"%s/",strargv4);//如/qxfile/zhrain24/
  strncat(strdstfilepath,strddatetime,4);     strcat(strdstfilepath,"/");  // 年的子目录 如/qxfile/zhrain24/2021/
  strncat(strdstfilepath,strddatetime+4,2);   strcat(strdstfilepath,"/");  // 月的子目录 如/qxfile/zhrain24/2021/01/
  strncat(strdstfilepath,strddatetime+6,2);   strcat(strdstfilepath,"/");  // 日的子目录 如/qxfile/zhrain24/2021/01/07/

  //目标文件重命名工作和目标文件存放的目录准备好之后
  char strfulldstfilename[301];//目标文件的全路径名
  memset(strfulldstfilename,0,sizeof(strfulldstfilename));
  snprintf(strfulldstfilename,300,"%s%s",strdstfilepath,strdstfilename);//如/qxfile/zhrain24/2021/01/07/zhrain24_20210106180000000.jpg
