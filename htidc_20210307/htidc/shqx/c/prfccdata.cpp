...
int main(int argc,char *argv[])
{
  if (argc!=5)
  {
    printf("\n本程序用于处理精细化预报数据，并保存到数据库的T_RFFC表中。\n");
    printf("/htidc/shqx/bin/psignallog 数据文件存放的目录 日志文件名 数据库连接参数 程序运行时间间隔\n");
    printf("例如：/htidc/shqx/bin/prfccdata /data/shqx/sdata/spcc /log/shqx/pspcc.log shtqapp/pwdidc 10\n");
    return -1;
  }
  ...
  while (true)
  {
    ...
    // 扫描数据文件存放的目录，只匹配"Z_SEVP_*RFFC*.TXT"
