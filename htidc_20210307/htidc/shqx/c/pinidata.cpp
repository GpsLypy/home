...
int main(int argc,char *argv[])
{
  if (argc!=5)
  {
   ...
   printf("例如：/htidc/shqx/bin/psurfdata /htidc/shqx/ini /log/shqx/ini.log shtq/pwdidc 10\n"); 
   return -1;
  }
  ...
  while (true)
  {
    ...
    if (Dir.OpenDir(argv[1],"*.ini",1000,true,true)==false)
    {
    ...
    while (true)
    {
      ...
      if (conn.m_state==0)//与数据库的连接状态，0-未连接，1-已连接。
      {
...
bool _psurfdata()
{
  ...
  while (true)
  {
   ...
   if (INIDATA.SplitBuffer(strBuffer)==false) { logfile.Write("%s\n",strBuffer); continue; }
    // 把结构体中的数据更新到T_SURFDATA表中
    //long rc=SURFDATA.InsertTable();
 long rc=INIDATA.InsertTable();
  ...
  logfile.WriteEx("成功(total=%d,insert=%d,update=%d,invalid=%d)。\n",INIDATA.totalcount,INIDATA.insertcount,INIDATA.updatecount,INIDATA.invalidcount);
