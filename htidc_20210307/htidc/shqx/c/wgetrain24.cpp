...
int main(int argc, char *argv[])
{
  if(argc!=4)
  {
    ...
    printf("中国天气网的降雨量实况图的url是http://products.weather.com.cn/product/Index/index/procode/JC_JSL_ZH.shtml\n");
    printf("如果中国天气网的url改变，程序也在做改动。\n");
    printf("logfilename 本程序的运行产生的日志文件名。\n");
    printf("tmpfilename 本程序运行产生的临时文件存放的目录。\n");
    printf("outputfilename 获取逐小时降雨量实况图存放的目录。\n\n");
    exit(1);
  }
  ...
  //true文件名，false目录名,创建用于存放临时文件和正式图片文件的目录
  ...
  while(true)
  {
   ...
   //system（）会调用fork（）产生子进程，由子进程来调用/bin/sh-c string来执行参数string字符串所代表的命令，此命令执行完后随即返回原调用的进程。在调用system期间SIGCHLD 信号会被暂时搁置，SIGINT和SIGQUIT 信号则会被忽略。
   ...
   if(File.Open(stroutputfile,"r")==false) {logfile.Write("File.Open(%s) failed.\n",stroutputfile); sleep(60); continue;}
...
bool GetURL(char *strBuffer,char *strURL,char *strFileName)
{
  char *start,*end;
  start=end=0;
  //字符串查找（strstr）,成功返回子串在字符串中第一次出现的位置
  if ((start=strstr(strBuffer,"http"))==0) return false;
  if ((end=strstr(start,"\""))==0) return false;//查询“”第一次出现位置

  strncpy(strURL,start,end-start);//把两个地址之间的内容给strURL
