...
bool _Parse(char *s,struct st_json *stjson,CJSONDATA *JSONDATAi,long &rc);
void EXIT(int sig);
int sum=0;
int aaa=0;
int main(int argc,char *argv[])
{
  ...
  while (true)
  {
    ...
    while (true)
    {
     ...
     // logfile.Write("totalcount=%d,insertcount=%d,updatecount=%d,invalidcount=%d",JSONDATA.totalcount,JSONDATA.insertcount,JSONDATA.updatecount,JSONDATA.invalidcount);
      sleep(atoi(argv[4]));
...
bool _pjsondata()
{
  ...
  while (true)
   {
    ...
    // logfile.Write("1->%s,%d\n",strBuffer,strlen(strBuffer));
     ...
     long rc=0;
     //记录拆分到结构体中
     if(_Parse(s,&stjson,&JSONDATA,rc)==false) {logfile.WriteEx("非法数据，不允许进库,处理其他数据...aaa=%d ",aaa++);continue;}   
  ...
  logfile.WriteEx("totalcount=%d,insertcount=%d,updatecount=%d,invalidcount=%d",JSONDATA.totalcount,JSONDATA.insertcount,JSONDATA.updatecount,JSONDATA.invalidcount);
  return true;
}
 


bool _Parse(char *s,struct st_json *stjson,CJSONDATA *JSONDATA,long &rc)
{
  //struct st_json stjson;
  ...
  JSONDATA->totalcount=array_size+JSONDATA->totalcount;
  for(i=0; i< array_size; i++) 
  {
   ...
   // logfile.Write("w11 is %s\n",js_w11->valuestring);   
    ...
    //if(root) cJSON_Delete(root);
     ...
     rc=JSONDATA->InsertTable(stjson);
