...
struct st_biz
{
  int  bizid;               // 业务代码
  char userid[51];          // 设备ID
  int  device;             // 用户的设备类型，0-未知；1-IOS；2-Andriod，2-鸿蒙。
...
void xmltobiz(char *strxmlbuffer)
{
  ...
  GetXMLBuffer(strxmlbuffer,"device",&stbiz.device);  
...
CTimer Timer;
  ...
  stmt.prepare("insert into T_USERINFO(userid,downtime,device,usertype,keyid) values(:1,sysdate,:2,:3,SEQ_USERINFO.nextval)");
