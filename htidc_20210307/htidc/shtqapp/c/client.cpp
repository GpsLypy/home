...
int main(int argc,char *argv[])
{
  ...
  if (TcpClient.ConnectToServer("192.168.65.130",5015)==false) { printf("conn failed.\n"); return -1; }
...
bool biz10000()
{
  ...
  if (TcpClient.Read(strRecvBuffer,20)==false)  { printf("1recv failed.\n"); return false; }
  printf("recv=%s=\n",strRecvBuffer);
...
bool biz10001()
{
  ...
  strcpy(strSendBuffer,"<bizid>10001</bizid><userid>52:54:00:83:0f:c1</userid><usertype>1</usertype><lat>20.234518</lat><lon>115.90832</lon><height>150.5</height>");
 ...
 // printf("send=%s=\n",strSendBuffer);
  ...
  {
    memset(strRecvBuffer,0,sizeof(strRecvBuffer));
    if (TcpClient.Read(strRecvBuffer,20)==false)  { printf("2recv failed.\n"); return false; }
   // printf("recv=%s=\n",strRecvBuffer);
